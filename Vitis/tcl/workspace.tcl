...
proc microblaze_boot_files {xsa_file app_name proc_instance board_name} {
  ...
  if {$mb_combine_bit_elf} {
    ...
    puts $wrapper_name
    puts $app_name
    puts "${vivado_folder}_i/$proc_instance"
    ...
    file copy "${vivado_path}/${vivado_folder}.runs/impl_1/${wrapper_name}.bit" "./boot/${board_name}"
...
proc create_vitis_ws {vivado_dirs} {
  ...
  foreach {xsa_file} $xsa_files {
    ...
    set files [glob -nocomplain -directory "./boot/$board_name" *]
    if {[llength $files] == 0} {
      # Get the processor instance name
      set proc_instance [get_processor_from_platform $platform_name]
      # For Microblaze designs
      if {[str_contains $proc_instance "microblaze"]} {
        print_sysproj $sysproj_name "Creating bitstream and copying to the ./boot/${board_name} directory."
        microblaze_boot_files $xsa_file $app_name $proc_instance $board_name
      # For Zynq and Zynq MP designs
      } else {
        print_sysproj $sysproj_name "Copying the BOOT.BIN file to the ./boot/${board_name} directory."
