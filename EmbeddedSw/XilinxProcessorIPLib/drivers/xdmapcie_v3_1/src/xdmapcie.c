							 ...
							 UINTPTR EffectiveAddress)
{
	...
	/* QDMA: config table has BaseAddress=ECAM, Ecam=CSR.
	 * The driver needs BaseAddress=CSR (for register access)
	 * and Ecam=ECAM (for config space access). Swap them. */
	{
		u64 ecam_addr = EffectiveAddress;
		InstancePtr->Config.BaseAddress = InstancePtr->Config.Ecam;
		InstancePtr->Config.Ecam = ecam_addr;
	}
	XDmaPcie_QdmaAddPgm(InstancePtr);
#elif defined(SDT) && defined(versal) && !defined(XDMA_PCIE_BRIDGE) && !defined(versal_2ve_2vm)
	InstancePtr->Config.BaseAddress= InstancePtr->Config.Ecam;
