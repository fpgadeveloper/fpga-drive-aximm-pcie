#----------------------------------------------------------------------------------------
# Constraints for Opsero FPGA Drive FMC Gen4 ref design for VC707-HPC2 using 1x SSD
#----------------------------------------------------------------------------------------

# SSD1 PCI Express reset (perst_0)
set_property PACKAGE_PIN AD40 [get_ports {perst_0[0]}]; # LA00_CC_P
set_property IOSTANDARD LVCMOS18 [get_ports {perst_0[0]}]

# SSD1 PE Detect (pedet_0, not connected in the design)
# set_property PACKAGE_PIN AD41 [get_ports {pedet_0[0]}]; # LA00_CC_N
# set_property IOSTANDARD LVCMOS18 [get_ports {pedet_0[0]}]

# Disable signal for 3.3V power supply for SSD2 (disable_ssd2_pwr)
set_property PACKAGE_PIN AC40 [get_ports disable_ssd2_pwr]; # LA07_P
...
# SSD1 ref clock
set_property PACKAGE_PIN K8 [get_ports {ref_clk_0_clk_p[0]}]; # GBTCLK0_M2C_P
set_property PACKAGE_PIN K7 [get_ports {ref_clk_0_clk_n[0]}]; # GBTCLK0_M2C_N
...
set_property LOC GTXE2_CHANNEL_X1Y16 [get_cells -hierarchical -filter { PRIMITIVE_TYPE =~ IO.GT.GTXE2_CHANNEL && NAME =~ "*axi_pcie_0*pipe_lane[0]*" }]
set_property LOC GTXE2_CHANNEL_X1Y17 [get_cells -hierarchical -filter { PRIMITIVE_TYPE =~ IO.GT.GTXE2_CHANNEL && NAME =~ "*axi_pcie_0*pipe_lane[1]*" }]
set_property LOC GTXE2_CHANNEL_X1Y18 [get_cells -hierarchical -filter { PRIMITIVE_TYPE =~ IO.GT.GTXE2_CHANNEL && NAME =~ "*axi_pcie_0*pipe_lane[2]*" }]
set_property LOC GTXE2_CHANNEL_X1Y19 [get_cells -hierarchical -filter { PRIMITIVE_TYPE =~ IO.GT.GTXE2_CHANNEL && NAME =~ "*axi_pcie_0*pipe_lane[3]*" }]
